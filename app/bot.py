...
class AnomalyView(discord.ui.View):
    ...
    async def _check_admin(self, interaction: discord.Interaction):
        ...
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("⛔ **Access Denied**: เฉพาะผู้ดูแลระบบ (Administrator) เท่านั้นที่มีสิทธิ์กดปุ่มนี้!", ephemeral=True)
            return False
        return True
...
async def on_message(message):
    ...
    if message.content.startswith('!analytics'):
        ...
        for row in data['uptime']:
            ...
            embed2.add_field(name = f"{emoji} {row[0]}", value = f"Uptime: **{uptime_pct}%**\nRecords: {row[2]:,}", inline = True)
